 ...
 * character arrays original and changed, and an integer array cost, where cost[i] represents the cost of 
...
Cost Matrix Initialization: Creates a matrix (c) to store the costs of transforming characters from the original array to the changed array.
Cost Assignment: Assigns costs between characters based on the original and changed arrays.
Floyd-Warshall Algorithm: Applies the Floyd-Warshall algorithm to compute the minimum costs between all characters.
Calculating Total Cost: Iterates through each character in the source and target strings, accumulating the total transformation cost.
Infinity Check: If the total cost exceeds a predefined infinity value, returns -1 indicating it's impossible to achieve the transformation.
Complexity
Time complexity:
O(n)
