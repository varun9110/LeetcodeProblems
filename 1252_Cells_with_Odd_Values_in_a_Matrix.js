 ...
 * There is also a 2D array indices where each indices[i] = [ri, ci] represents a 
...
var oddCells = function(m, n, indices) {
    ...
    for(let i=0; i<indices.length; i++){
        ...
        for(let j=0; j<m; j++){
            initialMatrix[j][column] += 1;
            if(initialMatrix[j][column] %2 ===1){
                count++;
            } else {
                count--;
            }
...
const oddCells = (r, c, indices) => {
    let oddRow = 0, oddCol = 0;
    const rCount = new Uint8Array(r), cCount = new Uint8Array(c);
