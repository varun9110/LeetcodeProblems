...
/**
 * Approach:
 * iterate through the student and check if the ith of student matches the current of sandwhich, if yes then make it -1. put a flag for each iteration
 * to check if at any iteration was there any event where no one could eat. if yes then no one else can eat.
 * return the count of non -1s
 */
...
var countStudents = function(students, sandwiches) {
    ...
    while(stud < students.length){
        if(sandwiches[sand] === students[stud]){
            flag = true;
            sand++;
            students[stud] = -1;
        }
        stud++;
        if(stud === students.length){
            if(flag){
                stud=0;
            }
            flag = false;
