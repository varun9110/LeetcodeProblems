...
If Alice and Bob reach the node simultaneously, they share the price/reward for opening the gate there. In other words, if the price to open the gate is c, 
then both Alice and Bob pay c / 2 each. Similarly, if the reward at the gate is c, both of them receive c / 2 each.
...
We first construct an adjacency list representation of the tree using the given edges.
Bob's Path Calculation:

We perform a DFS from bob to store the time at which Bob reaches each node (bobTime array).
Alice’s DFS Traversal:

We use another DFS from node 0 to track Alice’s traversal.
Alice collects the full reward if she reaches a node before Bob.
If Alice and Bob reach at the same time, Alice takes half the amount.
If Bob reaches first, Alice gets nothing.
We track the max income possible when Alice reaches a leaf node.
Complexity
Time complexity:

O(N) (since we perform DFS twice, once for Bob and once for Alice)
Space complexity:

O(N) (for adjacency list and time tracking arrays)
 */
