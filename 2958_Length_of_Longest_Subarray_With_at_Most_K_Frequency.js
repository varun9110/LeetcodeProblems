...
/**
 * Approach:
 * Use sliding window with a mapper to check on the frequency of each number. 
 * post this if the frequency of any number exceeds the k then increase the left unitl the frequency of right <=k 
 Keep finding the count
 */
 ...
 var maxSubarrayLength = function(nums, k) {
    ...
    while(right<nums.length){
        map[nums[right]] = (map[nums[right]]) ? map[nums[right]]+1 : 1;
        if(map[nums[right]] > k){
