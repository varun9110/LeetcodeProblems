...
Given a dictionary consisting of many roots and a sentence consisting of words separated by spaces, replace all the derivatives 
...
var replaceWords = function(dictionary, sentence) {
    ...
    for(let i=0; i<dictionary.length; i++){
        let dicLen = dictionary[i].length
        for(let j=0; j<sen.length; j++){
            if(dicLen <= sen[j].length && (dictionary[i] === sen[j].substring(0, dicLen))) {
                sen[j] = dictionary[i]
            }
        }
    }
