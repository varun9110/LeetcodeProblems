...
/**
 * Approach: 
 * Create a map and then iterate thorugh the array to check if num, num+diff and num+diff+diff exists
 * if exists then increase result
 * 
 * later return result
 */
...
var arithmeticTriplets = function(nums, diff) {
    ...
    for(let i=0; i<nums.length-2; i++){
        let num = nums[i];
        if(map[num] && map[num+diff] && map[num+(diff*2)]) {
            result++;
        }
    }
    return result
